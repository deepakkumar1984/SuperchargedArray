...
namespace System.ArrayExtension
{
    internal class Storage : RefCounter
    {
        ...
        public float GetElementAsFloat(long index)
        {
            unsafe
            {
                if (ElementType == DType.Single) return ((float*)buffer.ToPointer())[index];
                else if (ElementType == DType.Double) return (float)((double*)buffer.ToPointer())[index];
                else
        ...
        public void SetElementAsFloat(long index, float value)
        {
            unsafe
            {
                if (ElementType == DType.Single) ((float*)buffer.ToPointer())[index] = value;
                else if (ElementType == DType.Double) ((double*)buffer.ToPointer())[index] = value;
                else
                    throw new NotSupportedException("Element type " + ElementType + " not supported");
            }
        }
